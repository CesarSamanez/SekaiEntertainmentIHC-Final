function updateStocks(){
  ...
  var num=sv.substring(0,sv.indexOf("-"));
  sv=sv.substring(sv.indexOf("-")+1,sv.length);
  console.log("testprueba");
  while(num){
    var stock_element=document.getElementById("s"+num);
    var stock=stock_element.innerHTML;
    var sub_quant=document.getElementById("q"+num).value;
    stock_element.innerHTML=parseInt(stock)+parseInt(sub_quant);
    num=sv.substring(0,sv.indexOf("-"));
    sv=sv.substring(indexOf("-")+1,sv.length);
  }
}
...
function update(e, pri){
  ...
  console.log("test prueba");
    ...
    var stock_element=parseInt(document.getElementById("s"+pepe));
...
function updateTotal(){
  ...
  while(sv.indexOf("-")!=-1){
    ...
    ayuda2=document.getElementById("q"+subtotal_index).value;
    total_aux+=parseInt(ayuda);
    cant_aux+=parseInt(ayuda2);
    sv=sv.substring(sv.indexOf("-")+1,sv.length);
  }
  document.getElementById("total").value=total_aux;
  document.getElementById("cant").value=cant_aux;
}
function myFunction() {
