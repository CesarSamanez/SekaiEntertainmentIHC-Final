...
class UsersController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['forgot','logout', 'recover', 'register']);
    }
 ...
 // clear previous user input, since each Captcha code can only be validated once
    ...
    public function forgot(){
      if($this->request->is('post')){
        ...
        $changingUser=$this->Users->find()
        ->where([
          'email' => $em
        ]);
        $co="";
        foreach($changingUser as $c){
          for($i=0; $i<6; $i++){
            $co.=rand(0, 9);
          }
          $change=array('code'=>''.$co);
          $c = $this->Users->patchEntity($c, $change);
          $this->Users->save($c);
        }
        try {
            $res = $email->from('sekai.ent123@gmail.com')
                  ...
                  ->subject('Contact')
                  ->send($co." Es tu codigo para cambiar tu contraseña de SekaiEntertainment");
